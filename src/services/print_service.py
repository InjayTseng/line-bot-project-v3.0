...
class PrintService:
    ...
    async def create_print_job(self, print_settings=None):
        ...
        try:
            ...
            # 根據紙張大小選擇列印模式
            print_mode = "document"  # 預設使用文檔模式
            
            # 如果是照片紙，使用照片模式
            if default_settings.get('media_type') == 'mt_photopaper' or default_settings.get('media_size') in ['ms_a6', 'ms_4x6in']:
                print_mode = "photo"
